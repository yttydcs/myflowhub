...
func ParseEndpoint(raw string) (Endpoint, error) {
	...
	if len(raw) < len(prefix) || strings.ToLower(raw[:len(prefix)]) != prefix {
		return Endpoint{}, fmt.Errorf("%w", ErrEndpointSchemeInvalid)
	}
	body := raw[len(prefix):]
	parts := strings.SplitN(body, "?", 2)
	addrRaw := strings.Trim(strings.TrimSpace(parts[0]), "/")
	if addrRaw == "" {
	...
	if len(parts) == 2 {
		...
		if err != nil {
			return Endpoint{}, fmt.Errorf("rfcomm endpoint query invalid: %w", err)
		}
	}
