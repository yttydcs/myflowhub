...
OP_UNSUBSCRIBE = 6
OP_SUBSCRIBE_ACK = 7
OP_VARIABLE_SNAPSHOT = 8
OP_VARIABLE_UPDATE = 9
OP_STREAM_EVENT = 10
OP_STREAM_GAP = 11
OP_COMMAND_CALL = 12
OP_COMMAND_RESULT = 13
OP_ERROR = 14
OP_HEARTBEAT = 15

_HEADER = ">4sHBBBBHIQQQQq16s16sQ"


def _phase_allows(phase, operation):
    if operation == OP_JOIN:
        return phase == PHASE_REQUEST
    if operation in (OP_JOIN_ACK, OP_SUBSCRIBE_ACK, OP_VARIABLE_SNAPSHOT, OP_COMMAND_RESULT, OP_ERROR):
        return phase == PHASE_RESPONSE
    if operation in (3, 4, OP_VARIABLE_UPDATE, OP_STREAM_EVENT, OP_STREAM_GAP, OP_HEARTBEAT):
        return phase == PHASE_EVENT
    if operation in (OP_SUBSCRIBE, OP_UNSUBSCRIBE, OP_COMMAND_CALL):
...
def validate(envelope, max_payload=MAX_PAYLOAD):
    if envelope.get("version") != VERSION:
        raise ValueError("unsupported MFH3 version")
    ...
    requires_resource = OP_SUBSCRIBE <= operation <= OP_COMMAND_RESULT
    ...
    if operation in (OP_JOIN_ACK, OP_SUBSCRIBE_ACK, OP_VARIABLE_SNAPSHOT, OP_COMMAND_RESULT, OP_ERROR) and not any(correlation_id):
...
def encode(envelope, max_payload=MAX_PAYLOAD):
    ...
    payload = envelope.get("payload", b"")
    header = struct.pack(
        _HEADER, MAGIC, VERSION, envelope["phase"], envelope["operation"], len(content_type), len(schema), len(name), len(payload),
    ...
    return header + content_type + schema + name + payload


def decode(frame, max_payload=MAX_PAYLOAD):
    if len(frame) < HEADER_SIZE:
        raise ValueError("truncated MFH3 header")
    values = struct.unpack(_HEADER, frame[:HEADER_SIZE])
    magic, version, phase, operation, content_len, schema_len, name_len, payload_len = values[:8]
    if magic != MAGIC:
        raise ValueError("invalid MFH3 magic")
    if content_len > MAX_CONTENT_TYPE or schema_len > MAX_SCHEMA or name_len > MAX_RESOURCE_NAME or payload_len > max_payload:
        raise ValueError("frame metadata or payload exceeds embedded profile")
    total = HEADER_SIZE + content_len + schema_len + name_len + payload_len
    if len(frame) != total:
        raise ValueError("truncated or trailing MFH3 frame")
    offset = HEADER_SIZE
    ...
    envelope = {
        ...
        "source": values[8], "principal": values[9], "target": values[10], "topology_epoch": values[11],
        "deadline_unix_ms": values[12], "message_id": values[13], "correlation_id": values[14], "resource_owner": values[15],
        "content_type": content_type, "schema": schema, "resource_name": name, "payload": frame[offset:offset + payload_len],
    }
    validate(envelope, max_payload)
    return envelope
...
def read_frame(transport, max_payload=MAX_PAYLOAD):
    ...
    if len(header) != HEADER_SIZE or header[:4] != MAGIC:
        raise OSError("invalid or truncated MFH3 header")
    content_len, schema_len = header[8], header[9]
    name_len = struct.unpack(">H", header[10:12])[0]
    payload_len = struct.unpack(">I", header[12:16])[0]
    if payload_len > max_payload:
        raise ValueError("frame payload exceeds embedded profile")
    body_len = content_len + schema_len + name_len + payload_len
    if body_len > MAX_CONTENT_TYPE + MAX_SCHEMA + MAX_RESOURCE_NAME + max_payload:
