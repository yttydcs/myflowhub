...
class Client:
    ...
    def _base(self, phase, operation, target, message_id=None):
        return {
            "version": 1, "phase": phase, "operation": operation, "message_id": message_id or self._message_id(),
            ...
            "content_type": "application/json", "schema": "", "resource_name": "", "payload": b"",
        }
    ...
    def connect(self, permit=None):
        ...
        message = b"MFH3-JOIN" + struct.pack(">Q", self.identity.node_id) + nonce + struct.pack(">Q", self.epoch)
        ...
        ack_message = b"MFH3-ACK" + struct.pack(">Q", self.parent_id) + struct.pack(">Q", self.identity.node_id) + nonce + struct.pack(">Q", self.epoch)
        ...
        self.subscriptions = {}
        for owner, name, lease_ms, queue in previous:
            self.subscribe(owner, name, lease_ms, queue)
    ...
    def _resource(self, operation, owner, name):
        if not self.joined or owner <= 0 or not name:
            raise OSError("client is not joined or resource is invalid")
        envelope = self._base(protocol.PHASE_REQUEST, operation, owner)
        envelope["resource_owner"] = owner; envelope["resource_name"] = name
        return envelope

    def subscribe(self, owner, name, lease_ms=60000, queue=8):
        if lease_ms <= 0 or queue <= 0 or queue > 64:
            raise ValueError("subscription lease or queue is invalid")
        request = self._resource(protocol.OP_SUBSCRIBE, owner, name)
        request["schema"] = "subscribe.v1"; request["payload"] = _json({"lease_ms": lease_ms, "queue": queue}).encode()
        self._send(request)
        self.subscriptions[request["message_id"]] = (owner, name, lease_ms, queue)
    ...
    def unsubscribe(self, subscription_id):
        ...
        request = self._resource(protocol.OP_UNSUBSCRIBE, details[0], details[1]); request["correlation_id"] = subscription_id
        request["schema"] = "unsubscribe.v1"; request["payload"] = b"{}"; self._send(request); del self.subscriptions[subscription_id]

    def invoke(self, owner, name, schema, request, deadline_unix_ms):
        envelope = self._resource(protocol.OP_COMMAND_CALL, owner, name)
        ...
        while True:
            ...
            if response["operation"] == protocol.OP_ERROR:
                raise OSError("remote command error: %s" % response["payload"].decode())
            if response["operation"] != protocol.OP_COMMAND_RESULT:
                raise OSError("unexpected command response")
    ...
    def poll(self):
        ...
        if envelope["operation"] == protocol.OP_ERROR:
            return envelope
        if envelope["operation"] in (protocol.OP_VARIABLE_SNAPSHOT, protocol.OP_VARIABLE_UPDATE, protocol.OP_STREAM_EVENT, protocol.OP_STREAM_GAP):
