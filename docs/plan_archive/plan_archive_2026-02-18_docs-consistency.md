...
- 范围：
  - ✅ 更新 MyFlowHub3 控制面文档（`target.md` / `repos.md` / `plan.md` / `docs/protocol_map.md`）
...
### 验收标准（全局）
- `d:\project\MyFlowHub3\repos.md`、`target.md`、`plan.md`、`docs/protocol_map.md` 之间无互相矛盾的描述。
...
- 建立“文档分层 + 单一事实来源”的一致性策略：
  ...
  - `docs/protocol_map.md`：协议映射速查表（偏“具体 action 与数据结构在哪里”）
- 对“编号冲突”的处理原则：
  - 已归档的编号不追溯修改（保持可审计）。
...
> 结论：`repos.md` / `target.md` 的“Proto 为主、Server/protocol 为兼容壳”口径已基本一致；主要不一致集中在 **全局 `plan.md`** 与 **`docs/protocol_map.md`**，以及两个仓库 README 的过期路径/叙述。
...
- 涉及文件：
  ...
  - `d:\project\MyFlowHub3\docs\protocol_map.md`
- 验收：
  - `rg -n \"MyFlowHub-Server/protocol\\*\" plan.md docs/protocol_map.md` 不再出现“作为主入口”的误导语（允许在“兼容壳说明/历史归档”里出现）。
