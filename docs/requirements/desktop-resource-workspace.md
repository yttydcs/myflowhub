...
7. Node preview 至少显示身份、连接/健康状态、父子关系和资源摘要。
8. Renderer Registry 必须按 type/schema/capability 选择 preview、widget 和 editor/action，并为未知类型提供通用 inspector。
9. Workspace 必须支持添加、移除、重排和调整 Widget；拖放不是唯一入口。
10. View 只保存 Resource reference、renderer ID/version、layout 和局部展示设置，不复制 Resource 正文或权限。
11. View store 必须版本化、原子提交、按 Profile 隔离，并拒绝损坏或不兼容数据的静默覆盖。
12. Variable、Stream、Topic、Command、File 至少有可用的第一方 renderer；Media 在生产数据面完成前使用明确 capability/session placeholder。
13. 所有权限错误、资源消失、连接恢复、subscription gap 和 session 失败必须在对应 Widget 内明确显示。

## Non-functional Requirements

- 视觉遵循低噪声、充足留白、克制色彩、清晰层级和系统字体/CJK fallback。
- 支持系统浅色/深色主题和减少动画偏好。
- 树浏览、拖放、View 管理和核心操作必须具有键盘等价路径与可读辅助技术标签。
- 大型资源树按测量结果引入虚拟化；首屏和常规交互不得因为全树重渲染明显卡顿。
