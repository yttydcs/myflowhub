...
- 通知频道是同一版本化配置的一部分；节点消费父节点的 `notifications/events` Stream，并由 Windows 宿主呈现系统通知。
...
- 设备控制：可写指标由独立 `Command` 接收请求；Windows 在受控 platform adapter 内执行并回报实际状态。
...
- 链路：产品 runtime 只依赖可替换 `link.Driver`；当前命令行和 UI 使用 TCP，未来链路可在不改变资源模型的情况下替换 driver。
...
- Android 实现及专用 flashlight 指标已移除；[重新设计待办](../requirements/mobile-embedded-redesign.md)。
...
collector/actuator 可用 fake 测试；Windows collector 有平台 smoke；Windows UI 来自 canonical 契约并可重建。当前门禁包括 Wails production build、Go core/platform、Vitest 和真实 TCP 进程 e2e。
