...
## 目标
1) 在 `docs/specs/protocol_map.md` 的手写 Notes 区域补充 Management Nodes 语义说明：
...
- 文档只允许修改 `docs/specs/protocol_map.md` 的手写区域（`<!-- END GENERATED -->` 之后）。

## 验收标准
- `docs/specs/protocol_map.md` 明确写出上述语义，且不触碰生成区块。
...
- 已执行：`plan.md` → `docs/plan/plan_archive_2026-03-03_proto-management-children-only-prev.md`
- 验收条件：归档文件存在且可阅读。
- 回滚点：撤销该移动提交。
